...
protected:
    ...
    bool checkBalanced(Node<Key, Value>* node) const;
...
BinarySearchTree<Key, Value>::iterator::operator++()
{
    // If right subtree exists, go to the smallest value in that subtree.
    if (current_->getRight() != nullptr) {
        ...
        // Go up the tree until you've come from a node's left child.
        Node<Key, Value>* child = current_;
        Node<Key, Value>* parent = child->getParent();
...
void BinarySearchTree<Key, Value>::remove(const Key& key) {
    ...
    if (nodeToRemove->getLeft() && nodeToRemove->getRight()) {
        ...
        if (!pred) {
            // This shouldn't happen as nodeToRemove has two children
            return;
        }
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    ...
    if (current == nullptr || current->getLeft() == nullptr) return nullptr;
...
{
    // Implement a recursive delete starting from the root
    clearNode(root_);
    root_ = nullptr;  // Set root to nullptr after clearing the tree
}
...
void BinarySearchTree<Key, Value>::clearNode(Node<Key, Value>* node) {
    if (node != nullptr) {
        clearNode(node->getLeft());   // Delete left subtree
        clearNode(node->getRight());  // Delete right subtree
...
BinarySearchTree<Key, Value>::getSmallestNode() const
{
    ...
    return current;  // This could be nullptr if the tree is empty
}
 ...
 */
// Helper function to get the height of the tree from a node
template<typename Key, typename Value>
...
bool BinarySearchTree<Key, Value>::isBalanced() const {
    return checkBalanced(root_);
}

// Helper function to check if the tree is balanced from a node
template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::checkBalanced(Node<Key, Value>* node) const {
    if (node == nullptr) return true;
    int leftHeight = getHeight(node->getLeft());
    int rightHeight = getHeight(node->getRight());
    return abs(leftHeight - rightHeight) <= 1 && checkBalanced(node->getLeft()) && checkBalanced(node->getRight());
}
